...
class NeuralNavClient:
    ...
    def recommend(
        ...
        """
        # Step 1: Extract intent (skip when use_case + user_count overrides are both provided)
        if use_case_override is not None and user_count_override is not None:
            use_case = use_case_override
            user_count = user_count_override
            gpu_types = gpu_types_override if gpu_types_override is not None else []
        else:
