...
from rhoai_mcp.models.projects import ProjectCreate
...
def register_tools(mcp: FastMCP, server: "RHOAIServer") -> None:
    ...
    @mcp.tool()
    def get_project_status(name: str) -> dict[str, Any]:
        """Get comprehensive status of a Data Science Project.

        Returns detailed status including all workbenches, deployed models,
        pipeline server status, data connections, and storage.

        Args:
            name: The project (namespace) name.

        Returns:
            Comprehensive status information for the project.
        """
        ...
        models = inference_client.list_inference_services(name)

        # Get pipeline server
        pipeline_client = PipelineClient(server.k8s)
        pipeline_server = pipeline_client.get_pipeline_server(name)

        # Get data connections
        connection_client = ConnectionClient(server.k8s)
        connections = connection_client.list_data_connections(name)

        # Get storage
        storage_client = StorageClient(server.k8s)
        pvcs = storage_client.list_storage(name)
