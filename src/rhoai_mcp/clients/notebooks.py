...
class NotebookClient:
    ...
    def list_workbenches(self, namespace: str) -> list[Workbench]:
        ...
        return [
            Workbench.from_notebook_cr(
                nb, url=self._get_workbench_url(nb.metadata.name, namespace)
            )
            for nb in notebooks
        ]
    ...
    def get_workbench(self, name: str, namespace: str) -> Workbench:
        ...
        notebook = self._k8s.get(CRDs.NOTEBOOK, name=name, namespace=namespace)
        return Workbench.from_notebook_cr(
            notebook, url=self._get_workbench_url(name, namespace)
        )
    ...
    def create_workbench(self, request: WorkbenchCreate) -> Workbench:
        ...
        self._ensure_workbench_pvc(
            pvc_name, request.namespace, request.storage_size
        )

        # Create the notebook
        notebook = self._k8s.create(
            CRDs.NOTEBOOK, body=notebook_body, namespace=request.namespace
        )
    ...
    def delete_workbench(self, name: str, namespace: str) -> None:
        """Delete a workbench."""
        self._k8s.delete(CRDs.NOTEBOOK, name=name, namespace=namespace)
    ...
    def start_workbench(self, name: str, namespace: str) -> Workbench:
        ...
        if RHOAIAnnotations.NOTEBOOK_STOPPED in annotations:
            ...
            notebook = self._k8s.patch(
                CRDs.NOTEBOOK, name=name, body=patch_body, namespace=namespace
            )
    ...
    def stop_workbench(self, name: str, namespace: str) -> Workbench:
        ...
        patch_body = {
            "metadata": {
                "annotations": RHOAIAnnotations.notebook_stopped_annotation(timestamp)
            }
        }
    ...
    def _ensure_workbench_pvc(
        self, name: str, namespace: str, size: str
    ) -> None:
    ...
    def _build_notebook_cr(self, request: WorkbenchCreate) -> dict[str, Any]:
        ...
        return {
            "apiVersion": CRDs.NOTEBOOK.api_version,
            "kind": CRDs.NOTEBOOK.kind,
            "metadata": {
                "name": request.name,
