...
from rhoai_mcp_projects.client import ProjectClient
...
def register_resources(mcp: FastMCP, server: "RHOAIServer") -> None:
    ...
    def project_workbenches(name: str) -> list[dict[str, Any]]:
        ...
        """
        try:
            from rhoai_mcp_notebooks.client import NotebookClient
            ...
            return [
                {
                    ...
                    "url": wb.url,
                    "stopped_time": (
                        wb.stopped_time.isoformat() if wb.stopped_time else None
                    ),
                    "created": (
            ...
            return [{"error": "Notebooks plugin not available"}]
    ...
    def project_models(name: str) -> list[dict[str, Any]]:
        ...
        """
        try:
            from rhoai_mcp_inference.client import InferenceClient
            inference_client = InferenceClient(server.k8s)
            return inference_client.list_inference_services(name)
        except ImportError:
            return [{"error": "Inference plugin not available"}]
