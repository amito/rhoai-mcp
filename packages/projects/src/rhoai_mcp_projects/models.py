...
from rhoai_mcp_core.models.common import ResourceMetadata, ResourceSummary, ResourceStatus
...
class DataScienceProject(BaseModel):
    ...
    status: ResourceStatus = Field(
        ResourceStatus.READY, description="Project status"
    )
...
class ProjectCreate(BaseModel):
    ...
    enable_modelmesh: bool = Field(
        False, description="Enable ModelMesh (multi-model) serving"
    )
...
class ProjectUpdate(BaseModel):
    ...
    enable_modelmesh: bool | None = Field(
        None, description="Enable/disable ModelMesh serving"
    )
...
class ProjectStatus(BaseModel):
    ...
    pipeline_server: dict[str, Any] | None = Field(
        None, description="Pipeline server status"
    )
    ...
    storage: list[dict[str, Any]] = Field(
        default_factory=list, description="Storage (PVC) info"
    )
