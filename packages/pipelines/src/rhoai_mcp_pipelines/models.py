...
from rhoai_mcp_core.models.common import Condition, ResourceMetadata
...
class PipelineServer(BaseModel):
    ...
    status: PipelineServerStatus = Field(
        PipelineServerStatus.UNKNOWN, description="Server status"
    )
    api_server_ready: bool = Field(False, description="API server readiness")
    persistence_agent_ready: bool = Field(
        False, description="Persistence agent readiness"
    )
    ...
    object_store_available: bool = Field(
        False, description="Object store availability"
    )
...
class PipelineServerCreate(BaseModel):
    ...
    object_storage_secret: str = Field(
        ..., description="Name of secret with S3 credentials"
    )
